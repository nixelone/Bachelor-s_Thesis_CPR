...
def aggregate_second(buffer):
    """Returns (mean_bpm, majority_performer_id)."""
    valid_bpm = [v[0] for v in buffer if v[0] is not None]
    valid_ids = [v[1] for v in buffer if v[1] is not None]

    mean_bpm = int(round(sum(valid_bpm) / len(valid_bpm))) if valid_bpm else None
    active_id = max(set(valid_ids), key=valid_ids.count) if valid_ids else None
    
    return mean_bpm, active_id
...
def main(source, save_output=True, output_json_path="cpr_results.json", 
         output_video_path="cpr_analysis_output.mp4", buffer_seconds=2.5):
    ...
    while cap.isOpened():
        ...
        sec_buffer.append((final_output, active_id if final_output is not None else None))
        if frame_count >= (current_sec + 1) * fps:
            bpm, yolo_id = aggregate_second(sec_buffer)
            
            perf_id = None
            if yolo_id is not None:
                if yolo_id not in yolo_to_performer:
                    yolo_to_performer[yolo_id] = next_performer_id
                    next_performer_id += 1
                perf_id = yolo_to_performer[yolo_id]
        ...
        # Keep the visualizer but allow fast-quitting the CURRENT video by pressing 'q'
    ...
    print(f"Results saved to {output_json_path} and {output_video_path}\n")


def process_batch(input_folder="experiment_videos", output_folder="output", buffer_seconds=2.5):
    """Finds all videos in input_folder and processes them into output_folder."""
    ...
    # Gather all video files (add or remove extensions as needed)
    ...
    for filename in video_files:
        ...
        # Get filename without extension to construct corresponding json path
        name_no_ext = os.path.splitext(filename)[0]
        
        # Construct the requested output paths
        out_video_path = os.path.join(output_folder, f"output_{filename}")
        out_json_path  = os.path.join(output_folder, f"output_{name_no_ext}.json")
...
if __name__ == "__main__":
    # Runs on the whole folder instead of a single hardcoded video
    process_batch(input_folder="experiment_videos", output_folder="output_2", buffer_seconds=2.5)